...
export default function CreditSimular() {
  ...
  const renderInput = (key: string, label: string, props = {}) => (
    <div className="space-y-1">
      ...
      <Input id={key} value={formValues[key] || ""} onChange={(e) => handleChange(key, e.target.value)} {...props} />
    </div>
  );
  ...
  const camposPorProduto = {
    1: (
      <>
        <Combobox data={tipoPessoa} displayField="name" value={formValues.tipoPessoa} onChange={(val) => handleChange("tipoPessoa", val)} label="Tipo de Pessoa" searchFields={["name"]} />
        <Combobox data={tomador} displayField="name" value={formValues.tomador} onChange={(val) => handleChange("tomador", val)} label="Tomador" searchFields={["name"]} />
        {renderInput("cpf", "CPF")}
        {renderInput("saldoFgts", "Saldo FGTS")}
        <Combobox data={aniversario} displayField="name" value={formValues.aniversario} onChange={(val) => handleChange("aniversario", val)} label="Mês Aniversário" searchFields={["name"]} />
    ...
    2: (
      <>
        <Combobox data={tipoPessoa} displayField="name" value={formValues.tipoPessoa} onChange={(val) => handleChange("tipoPessoa", val)} label="Tipo de Pessoa" searchFields={["name"]} />
        <Combobox data={tipoCalculo} displayField="name" value={formValues.tipoCalculo} onChange={(val) => handleChange("tipoCalculo", val)} label="Tipo de cálculo" searchFields={["name"]} />
        ...
        <Combobox data={baseCalculo} displayField="name" value={formValues.baseCalculo} onChange={(val) => handleChange("baseCalculo", val)} label="Base de cálculo" searchFields={["name"]} />
        {renderInput("pagamentoCada", "Pagamento à cada")}
        <Combobox data={periodicidadePagamento} displayField="name" value={formValues.periodicidadePagamento} onChange={(val) => handleChange("periodicidadePagamento", val)} label="Periodicidade do Pagamento" searchFields={["name"]} />
        {renderDateInput("dataInicio", "Data de Início")}
        {renderDateInput("dataPrimeiroPagamento", "Data do primeiro pagamento")}
        {renderInput("corban", "CORBAN")}
        {renderInput("garantiaFiduciaria", "Garantia fiduciária")}
        <Combobox data={indexadorPosFixado} displayField="name" value={formValues.indexadorPosFixado} onChange={(val) => handleChange("indexadorPosFixado", val)} label="Indexador Pós-Fixado" searchFields={["name"]} />
